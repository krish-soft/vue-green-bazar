<template>
    <BaseContainer heading="Product Management">
            ...
            <table id="datatable" class="table table-hover table-striped table-bordered align-middle">
                ...
                <tbody v-if="mListData.length">
                    <tr v-for="(row, i) in mListData" :key="row.id">
                        ...
                        <td>{{ row?.category?.name }} ({{ row?.category?.category_code }})</td>
                        <td>{{ row.hsn }}</td>
                        <td>{{ row.upc }}</td>
                        <td>{{ row.sku }}</td>
    ...
    <BaseModal ref="aModal" icon="fas fa-tags">
        ...
        <form id="newForm" @submit.prevent="submitForm">
            ...
            <div class="mb-3">
                ...
                <BaseAutoCompleteSelect label="Category" v-model="form.category_id" :options="categoryList"
                    :label-Key="['name', 'category_code']" value-Key="id" placeholder="Select Product Category"
                    required />
            </div>
            ...
            <div class="mb-3">
                <BaseInput label="HSN" v-model.trim="form.hsn" placeholder="Enter HSN" required />
            </div>
...
async function loadList() {
    mListData.value = await fetchProducts();   // ✅ NO CHECK
}

async function loadCategoryList() {
    if (categoryList.value.length > 0) return;
    categoryList.value = await fetchProductCategories({ is_active: true });   // ✅ NO CHECK
}
