<template>
    ...
    <BaseContainer heading="Demand Order Details">
            ...
            <div v-if="orderDetails" class="container-fluid px-0">
                ...
                <div class="mb-4">
                    ...
                    <table class="table table-sm table-bordered table-striped align-middle mb-0">
                        ...
                        <tbody>
                            <tr v-for="(charge, index) in orderDetails.demand_order_charges" :key="charge.id">
                                <td>{{ index + 1 }}</td>
...
async function loadDetails() {
    // Fetch demand order details by ID
    const data = await fetchDemandOrderDetails(orderId.value);
    if (!data) {
        return;
    }
...
const orderChargesTotals = computed(() => {
    ...
    const charges = orderDetails.value.demand_order_charges || []
...
const orderItemsTotals = computed(() => {
    ...
    const items = orderDetails.value.demand_order_items || []
